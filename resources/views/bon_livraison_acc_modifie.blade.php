@extends('base')
@section('contenue')
@foreach($lignelivraisons as $dev)
<form action = "/ligne_livraison/accepted/{{$dev->id}}" method = "post" class="form-horizontal form-label-left">
@csrf <!-- {{ csrf_field() }} -->
...
<div class="col-md-9 col-sm-9 col-xs-12">
  ...
  <option value="{{$dev->bon_livraison_id}}">{{$dev->bon_livraison_id}}</option>
  <option value="Aucune">..........</option>
   @foreach ($bonlivraisons as $devi)
